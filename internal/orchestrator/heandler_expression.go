...
func (apiCfg *apiConfig) handlerProcessExpression(w http.ResponseWriter, r *http.Request) {
	...
	expr := entities.FormatExpression(request.Expression)

	if entities.IsValid(expr) {
		expressionFromDB, err := apiCfg.DB.GetExpressionByID(r.Context(), entities.CreateIdempotentKey(expr))
...
func registrateNewExpression(apiCfg *apiConfig, r *http.Request, expr []string) (int, []database.Subexpression, error) {
	...
	err := entities.ProcessExpression(expr, subexpressionMap)
	if err != nil {
		return 400, nil, err
	}

	newExpression, err := apiCfg.DB.CreateExpression(r.Context(), database.CreateExpressionParams{
		ID:                   entities.CreateIdempotentKey(expr),
...
func sendSubexpressions(tasks []entities.AgentsTask, durations []database.OperatorsDuration) error {
...
func prepareSubexpressionsForSend(apiCfg *apiConfig, subexpressions []database.Subexpression) []entities.AgentsTask {
	agentsTasks := make([]entities.AgentsTask, 0)
	...
	for _, subexpression := range subexpressions {
		...
		specialOperands := entities.GetSubsexprNumbersBySubsexpr(subexpression.SubexpressionBody)
		...
		if !entities.IsContainsUnknownVar(subexpression.SubexpressionBody) && subexpression.SubexpressionStatusID == 2 {
			...
			agentsTasks = append(agentsTasks, entities.ConvertSubexpressionToAgentsTask(subexpression))
		}
	}
